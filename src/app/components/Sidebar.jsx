...
import { AiOutlineSearch, AiOutlineHome, AiOutlineClose } from "react-icons/ai"; // Import the close icon
...
const Sidebar = ({ open, setOpen }) => {
    ...
    return (
        <div className="flex">
            <div
                className={`bg-slate-100 min-h-screen p-5 pt-8 duration-300 fixed z-10 md:relative md:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"
                    } md:w-80 w-60`}
            >
                {/* Close icon for small screens */}
                <AiOutlineClose
                    className="text-2xl absolute top-4 right-4 cursor-pointer block sm:hidden"
                        ...
                        <div key={index}>
                            <Link href={menu.path || "#"} passHref>
                                <li
                                    ...
                                    ${menu.spacing ? "mt-9" : "mt-2"} ${activeMenu === menu.title
                                            ? "bg-slate-300 font-bold text-black"
                                            : "text-gray-700"
                                        }`}
                                >
